    ...
    >
        ...
        >
            ...
            <div class="-mt-9 grid max-w-[291px] translate-y-9 content-start gap-2.5 bg-white p-2.5 transition-transform duration-300 ease-out group-hover:-translate-y-0 group-hover:rounded-t-lg max-md:relative max-md:mt-0 max-md:translate-y-0 max-md:gap-0 max-md:px-0 max-md:py-1.5 max-sm:min-w-[170px] max-sm:max-w-[192px]">
                ...
                <!-- Price Breakdown (Koko Installments) -->
                <div 
                    class="product-price-installments-not-in-variation-single text-sm text-zinc-500 max-sm:text-xs"
        ...
        >
            ...
            <div class="grid content-start gap-4">
                ...
                <!-- Price Breakdown (Koko Installments) -->
                <div 
                    class="product-price-installments-not-in-variation-single text-sm text-zinc-500"
                    v-if="installmentPrice"
                >
                    or 3 X <b>@{{ formatCurrency(installmentPrice) }}</b> with 
                    <span>
    ...
    <script type="module">
        app.component('v-product-card', {
            ...
            computed: {
                installmentPrice() {
                    if (!this.product) {
                        console.log('Koko Installment: No product data');
                        return null;
                    }
                    ...
                    // First try: product.prices.final.price (standard structure)
                    if (this.product.prices && this.product.prices.final && typeof this.product.prices.final.price === 'number') {
                        finalPrice = this.product.prices.final.price;
                        console.log('Koko Installment: Found price in product.prices.final.price', finalPrice);
                    } 
                    // Second try: product.price.final.price (bundle products)
                    else if (this.product.price && this.product.price.final && typeof this.product.price.final.price === 'number') {
                        finalPrice = this.product.price.final.price;
                        console.log('Koko Installment: Found price in product.price.final.price', finalPrice);
                    }
                    // Third try: Extract from min_price formatted string
                    else if (this.product.min_price) {
                        // Extract numeric value from formatted price string (handles Rs.1,234.56 or $1,234.56)
                        const priceStr = this.product.min_price.toString().replace(/[^\d.]/g, '');
                        const parsed = parseFloat(priceStr);
                        if (!isNaN(parsed) && parsed > 0) {
                            finalPrice = parsed;
                            console.log('Koko Installment: Extracted price from min_price', finalPrice);
                        }
                    }
                    // Fourth try: Try to extract from price_html by parsing
                    else if (this.product.price_html) {
                        // Try to find price in HTML - look for common patterns
                        const htmlStr = this.product.price_html;
                        // Match patterns like Rs.1,234.56 or $1,234.56
                        const priceMatch = htmlStr.match(/[\d,]+\.?\d*/);
                        if (priceMatch) {
                            const parsed = parseFloat(priceMatch[0].replace(/,/g, ''));
                            if (!isNaN(parsed) && parsed > 0) {
                                finalPrice = parsed;
                                console.log('Koko Installment: Extracted price from price_html', finalPrice);
                            }
                        }
                    }
                    
                    if (!finalPrice) {
                        console.log('Koko Installment: Could not find price. Product data:', {
                            hasPrices: !!this.product.prices,
                            hasPrice: !!this.product.price,
                            hasMinPrice: !!this.product.min_price,
                            hasPriceHtml: !!this.product.price_html,
                    ...
                    if (finalPrice && !isNaN(finalPrice) && finalPrice > 0) {
                        ...
                        console.log('Koko Installment: Calculated installment', installment);
                        return installment;
                    }
            ...
            methods: {
                formatCurrency(amount) {
                    // Format the amount as currency (e.g., Rs.966.67)
