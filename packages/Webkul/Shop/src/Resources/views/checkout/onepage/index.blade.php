    ...
    @pushOnce('scripts')
        ...
        <script type="module">
            app.component('v-checkout', {
                ...
                computed: {
                    kokoInstallmentPrice() {
                        if (!this.cart || !this.cart.grand_total || parseFloat(this.cart.grand_total) <= 0) {
                            return '';
                        }
                        const installmentPrice = parseFloat(this.cart.grand_total) / 3;
                        return this.$shop.formatPrice(installmentPrice);
                    },
                    ...
                    kokoInstallmentAmount() {
                        ...
                        return (parseFloat(this.cart.grand_total) / 3).toFixed(2);
                    },

                    showKokoOption() {
                        return this.cart && this.cart.grand_total && parseFloat(this.cart.grand_total) > 0;
                    }
                },
                ...
                methods: {
                    formatKokoInstallment(grandTotal) {
                        if (!grandTotal || grandTotal <= 0) {
                            return '';
                        }
                        const installmentPrice = parseFloat(grandTotal) / 3;
                        return this.$shop.formatPrice(installmentPrice);
                    },
