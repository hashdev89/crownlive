...
@push('meta')
    ...
    <meta property="og:url" content="{{ url('/' . $product->url_key) }}" />
@endPush
    ...
    @pushOnce('scripts')
                        ...
                        <div class="mt-12 flex gap-9 max-1180:flex-wrap max-lg:mt-0 max-sm:gap-y-4">
                            ...
                            <div class="relative max-w-[590px] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                ...
                                @php
                                    ...
                                    // For configurable products, use 'regular' price, for others use 'final' price
                                    $finalPrice = $prices['final']['price'] ?? $prices['regular']['price'] ?? null;
                                    // Convert to current currency if needed (price is already converted in getProductPrices)
                                    $installmentPrice = $finalPrice && $finalPrice > 0 ? ($finalPrice / 3) : null;
                                    $formattedInstallmentPrice = $installmentPrice ? core()->currency($installmentPrice) : null;
                                @endphp

                                <!-- KOKO Payment Option -->
                                @if ($installmentPrice && $installmentPrice > 0)
                                <div 
                                    id="koko-installment-display"
                                    class="product-price-installments-not-in-variation-single mt-2 text-sm text-zinc-500 max-sm:text-xs"
                                >
                                    or 3 X <b id="koko-installment-price">{{ $formattedInstallmentPrice }}</b> with 
                                    <span>
                                        <img 
                                        ...
                                        <img 
                                            id="koko-info-icon"
                                            data-price="{{ number_format($installmentPrice, 2) }}" 
                                ...
                                <div class="flex max-w-[470px] gap-4 max-sm:mt-4">
                                {{-- Whatsapp Help Button --}}
                                @php
                                    $currentUrl = urlencode(url()->current());
                                    $waMessage = urlencode("Hello! I need help with this product: " . url()->current());
                                    $waLink = "https://wa.me/94777604941?text={$waMessage}";
                                @endphp
                                
                                <a href="{{ $waLink }}"
                                   target="_blank"
                                   class="fragrance-help-btn">
                                    💬 Ask a Fragrance Expert
                                </a>
                                </div>
                                <!-- Product Actions and Qunatity Box -->
                                <div class="flex max-w-[470px] gap-4 max-sm:mt-4 add_to_cart--container">
        ...
        <script type="module">
            app.component('v-product', {
                ...
                mounted() {
                    // Listen for configurable variant selection to update KOKO installment
                    this.$emitter.on('configurable-variant-selected-event', (variantId) => {
                        this.updateKokoInstallment(variantId);
                    });
                    
                    // Initial update after a short delay to ensure DOM is ready
                    setTimeout(() => {
                        this.updateKokoInstallment(0);
                    }, 100);
                },

                methods: {
                    updateKokoInstallment(variantId) {
                        const kokoDisplay = document.getElementById('koko-installment-display');
                        const kokoPrice = document.getElementById('koko-installment-price');
                        const kokoInfo = document.getElementById('koko-info-icon');
                        
                        if (!kokoDisplay || !kokoPrice) {
                            return;
                        }

                        let finalPrice = null;

                        // Get price from the final-price element in the DOM
                        const finalPriceElement = document.querySelector('.final-price');
                        if (finalPriceElement) {
                            // Extract numeric price from formatted string
                            const priceText = finalPriceElement.textContent || finalPriceElement.innerText;
                        ...
                        if (finalPrice && finalPrice > 0) {
                            const installmentPrice = finalPrice / 3;
                            const formattedInstallment = this.$shop.formatPrice(installmentPrice);
                            
                            kokoPrice.textContent = formattedInstallment;
                            if (kokoInfo) {
                                kokoInfo.setAttribute('data-price', installmentPrice.toFixed(2));
                            }
                            kokoDisplay.style.display = '';
                        } else {
                            kokoDisplay.style.display = 'none';
                        }
                    },
