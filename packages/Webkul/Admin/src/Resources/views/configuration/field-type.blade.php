@php
    ...
    // Remove required_if and mimes rules from frontend validation as VeeValidate doesn't support them properly
    // Backend will handle these validations
    $validations = $field->getValidations();
    if ($validations) {
        // Remove required_if rules
        $validations = preg_replace('/required_if:[^,|]+(?:,|$)/', '', $validations);
        // Remove mimes rules (file type validation)
        $validations = preg_replace('/mimes:[^,|]+(?:,|$)/', '', $validations);
        // Clean up any leftover commas or pipes
        $validations = preg_replace('/[,|]+/', ',', $validations);
